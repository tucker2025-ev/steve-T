...
public class ChargePointRepositoryImpl implements ChargePointRepository {
    ...
    @Override
    public List<ChargePoint.Overview> getChargePointDetails() {
        return ctx.select(
                        CHARGE_BOX.CHARGE_BOX_PK,
                        CHARGE_BOX.CHARGE_BOX_ID,
                        CHARGE_BOX.DESCRIPTION,
                ...
                .from(CHARGE_BOX)
                .fetch()
                .map(record -> ChargePoint.Overview.builder()
                        .chargeBoxPk(record.get(CHARGE_BOX.CHARGE_BOX_PK))
                        .chargeBoxId(record.get(CHARGE_BOX.CHARGE_BOX_ID))
                        .description(record.get(CHARGE_BOX.DESCRIPTION))
                        .ocppProtocol(record.get(CHARGE_BOX.OCPP_PROTOCOL))
                        .lastHeartbeatTimestamp(record.get(CHARGE_BOX.LAST_HEARTBEAT_TIMESTAMP) != null
                                ? record.get(CHARGE_BOX.LAST_HEARTBEAT_TIMESTAMP).toString("yyyy-MM-dd HH:mm:ss")
                                : null)
                        .lastHeartbeatTimestampDT(record.get(CHARGE_BOX.LAST_HEARTBEAT_TIMESTAMP))
                        .build()
                );
    }
