...
public enum SteveConfiguration {
    ...
    private final DB db;
    private final DB db2;
    private final DB db3;
    ...
    SteveConfiguration() {
        ...
        db3 = DB.builder()
                .ip(p.getString("db3.ip"))
                .port(p.getInt("db3.port"))
                .schema(p.getString("db3.schema"))
                .userName(p.getString("db3.user"))
                .password(p.getString("db3.password"))
                .sqlLogging(p.getBoolean("db3.sql.logging"))
                .build();


        db2 = DB.builder()
                .ip(p.getString("db2.ip"))
                .port(p.getInt("db2.port"))
                .schema(p.getString("db2.schema"))
                .userName(p.getString("db2.user"))
                .password(p.getString("db2.password"))
                .sqlLogging(p.getBoolean("db2.sql.logging"))
                .build();
