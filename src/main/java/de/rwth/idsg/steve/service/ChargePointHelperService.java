...
public class ChargePointHelperService {
    ...
    public Statistics getStats() {
        ...
        List<ConnectorStatus> latestListNew = homeService.findCConnectedChargeBoxConnectors(latestList);
        stats.setStatusCountMap(ConnectorStatusCountFilter.getStatusCountMap(latestListNew));
        return stats;
    }
...
//  add extra for scheduleCharging
    public boolean isOnline(final String chargeBoxId) {
        return getConnectedJsonChargeBoxId().contains(chargeBoxId);
    }

    private Set<String> getConnectedJsonChargeBoxId() {
        Map<String, Deque<SessionContext>> ocpp12Map = ocpp12WebSocketEndpoint.getACopy();
        Map<String, Deque<SessionContext>> ocpp15Map = ocpp15WebSocketEndpoint.getACopy();
        Map<String, Deque<SessionContext>> ocpp16Map = ocpp16WebSocketEndpoint.getACopy();

        return new HashSet<>(extractIds(Arrays.asList(ocpp12Map, ocpp15Map, ocpp16Map)));
    }
