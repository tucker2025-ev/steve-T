...
//        if (!activeTransactionIsConnector(connectorId,chargeBoxId)){
//            log.error("Connector [{}] on ChargeBox [{}] has an open transaction. Cannot proceed.", connectorId, chargeBoxId);
//            return new IdTagInfo().withStatus(AuthorizationStatus.CONCURRENT_TX);
//
//        }
        if (record == null) {
...
//        }
    ...
    private boolean activeTransactionIsConnector(Integer connectorId, String chargeBoxId) {
        ...
        String latestStatus = dslContext
                .select(CONNECTOR_STATUS.STATUS)
        ...
        if (latestStatus.equalsIgnoreCase("Preparing") || latestStatus.equalsIgnoreCase("finishing") ){
            return true;
        }
        return false;
    }
