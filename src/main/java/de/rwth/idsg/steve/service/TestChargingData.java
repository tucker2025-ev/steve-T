...
import de.rwth.idsg.steve.service.dto.VehileDTO;
...
public class TestChargingData {
    ...
    private static final Table<?> USER_AND_VEHICLE_DETAILS_VIEW =
            DSL.table("bigtot_cms.view_user_with_full_vehicle_info");
    ...
    private static final Field<String> VEHICLE_NUMBER = DSL.field("vehicle_name", String.class).as("number");
    ...
    public void liveChargingData(final String chargeBoxId,
        ...
        try {
            /* -------------------------------------------------
             * 1️⃣ Fetch Charge Point Details
             * ------------------------------------------------- */
            ...
            if (chargePointDTO == null) {
                log.warn("⚠️ Charge point not found. chargeBoxId={}, connectorId={}",
                        chargeBoxId, connectorId);
                return;
            }

            /* -------------------------------------------------
             * 2️⃣ Fetch User & Vehicle Details (may be empty)
             * ------------------------------------------------- */
            ...
            List<VehileDTO> vehicles = php
                    ...
                    .from(USER_AND_VEHICLE_DETAILS_VIEW)
                    .where(IDTAG.eq(idTag))
                    .fetchInto(VehileDTO.class);

            /* -------------------------------------------------
             * 3️⃣ Resolve User (Guest fallback)
             * ------------------------------------------------- */
            UserDTO userDTO = users.isEmpty()
                    ? guestUser()
                    : users.stream()
            ...
            /* -------------------------------------------------
             * 4️⃣ Resolve Vehicle (Guest fallback)
             * ------------------------------------------------- */
            VehileDTO vehileDTO = vehicles.isEmpty()
                    ? guestVehicle()
                    : vehicles.stream()
            ...
            secondary.insertInto(LIVE_CHARGING_DATA)
                    ...
                    .set(LIVE_CHARGING_DATA.USER_ADDRESS_TWO, userDTO.getAddressTwo())
                    ...
                    // Vehicle info (safe for Guest)
                    .set(LIVE_CHARGING_DATA.CHARGING_VEHICLE_NAME, vehileDTO.getName())
                    .set(LIVE_CHARGING_DATA.CHARGING_VEHICLE_NUMBER, vehileDTO.getNumber())
                    .set(LIVE_CHARGING_DATA.CHARGING_VEHICLE_MODEL, vehileDTO.getModel())
                    .set(LIVE_CHARGING_DATA.CHARGING_VEHICLE_TYPE, vehileDTO.getType())
                    .set(LIVE_CHARGING_DATA.CHARGING_VEHICLE_IMG_URL, vehileDTO.getImgUrl())
                    .set(LIVE_CHARGING_DATA.CHARGING_VEHICLE_HUB_IMAGE, vehileDTO.getHubImage())
                    .execute();

        } catch (Exception e) {
            log.error("❌ Error in liveChargingData(): chargeBoxId={}, txId={}",
                    chargeBoxId, transactionId, e);
        }
    }
    ...
    private VehileDTO guestVehicle() {
        VehileDTO v = new VehileDTO();
